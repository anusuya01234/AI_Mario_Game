...
function changeGameStatud(character){
 //1...remove 3 conditions and Add 1st condition of checking if the play button is pressed, Add the 2nd condition of checking if the posenet model is loaded.
...
function getCoins(coin,character){
  if( character.overlap(coin) && character.live && coin.get==false){
    character.coins+=1;
    coin.get=true;
    mario_coin.play();//d..playing the sound of mario collecting coin
  };
}
...
function manualControl(character){
  ...
  if(character.live){
    //3...change the “if condition” to check x coordinate of nose
    if(noseX < 300){
      character.velocity.x-=gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(-1);
    }
//4...change the control of the mario for moving right - from using “right arrow key” To “x coordinate of nose
    if(noseX > 300){
        character.velocity.x+=gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(1);
...
function jumping(character){
  //5...checking y coordinate of nose is in PART A
	if( (noseY < 168  &&character.live) || (touchIsDown&&character.live) ){
    character.velocity.y+=gameConfig.jump;
    mario_jump.play();//b...playing the sound of mario jumping
	}
}
...
function StepOnEnemy(obj1,obj2){
    ...
    mario_kick.play();//h..playing the music of mario killing the enemies.
	}
}
...
function die(character){
    ...
    //k..add a “if condition” to check “if” mario live is greater than 0 then only play mario dying music
    if(character.liveNumber > 0)
    {
      mario_die.play();//j..playing mario die music.
    }
}
...
function checkStatus(character){    
  ...
  if(character.live==false && character.liveNumber==0){
    ...
    mario_gameover.play();//f..playing the sound of the game over 
  }
